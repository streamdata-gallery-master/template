---
name: Nordea
description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
  way to try out the upcoming OBI. This API is a sandbox version, and its purpose
  is to make developers familiar with the upcoming OBI production release. Moreover,
  it allows the developers to experiment and build applications which use the OBI
  before its official release.nbsp;
image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
x-kinRank: "8"
x-alexaRank: ""
tags:
- UK Banks
created: "2018-02-19"
modified: "2018-02-19"
url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/apis.yaml
specificationVersion: "0.14"
apis:
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/accounts-get.md
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/accounts-accountid-get.md
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/accounts-accountid-transactions-get.md
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/payments-sepa-get.md
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/payments-sepa-post.md
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/payments-sepa-paymentid-get.md
- name: Nordea
  description: The Nordeas Open Banking Initiative (OBI) API, offers a safe and simple
    way to try out the upcoming OBI
  image: http://kinlane-productions.s3.amazonaws.com/api-evangelist-site/company/logos/nordea-bank-logo.png
  humanURL: https://nordeaopenbanking.com
  baseURL: https://nordeaopenbanking.com
  tags:
  - Gallery
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery/apis/master/_uk_banks/nordea/payments-sepa-paymentid-confirm-put.md
x-common:
- type: x-blog
  url: https://medium.com/@NordeaOpenBanking
- type: x-blog-rss
  url: https://medium.com/feed/@NordeaOpenBanking
- type: x-developer
  url: https://developer.nordeaopenbanking.com/
- type: x-documentation
  url: https://developer.nordeaopenbanking.com/app/docs
- type: x-email
  url: openbankingsupport@nordea.com
- type: x-github
  url: https://github.com/NordeaOSS
- type: x-privacy-policy
  url: https://developer.nordeaopenbanking.com/app/page/privacy
- type: x-terms-of-service
  url: https://developer.nordeaopenbanking.com/app/page/terms
- type: x-twitter
  url: https://twitter.com/NordeaAPI
- type: x-website
  url: https://nordeaopenbanking.com
- type: x-portal
  url: https://nordeaopenbanking.com
include: []
maintainers:
- FN: Kin Lane
  x-twitter: apievangelist
  email: info@apievangelist.com
---